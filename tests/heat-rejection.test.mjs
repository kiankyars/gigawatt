...
test('chapter covers all D11 objectives and preserves original outdoor deep links',()=>{
 assert.equal(scenes.length,17);assert.equal(new Set(scenes.map(s=>s.id)).size,17);
...
function playerAt(hash){
 class Element{
  ...
  getAttribute(key){return this.attributes[key];}focus(){}addEventListener(name,fn){this.listeners[name]=fn;}
 }
