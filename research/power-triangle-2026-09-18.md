...
Added `#power-triangle` immediately after `#apparent-power-beer` and before `#power-factor-explained`. Chapter 6 now has 30 slides; the new scene is slide 24.

## Teaching purpose

The beer picture is a memory cue, but its stacked heights could imply that real and reactive power add arithmetically. The native SVG instead uses a geometrically accurate right triangle: 900 kW horizontally, 675 kvar vertically and 1,125 kVA along the hypotenuse. The same load appears in the following transformer example. The visible equations are `S² = P² + Q²` and `PF = P / S = 0.80`.
...
This triangle assumes sinusoidal voltage and current. Its angle is the phase displacement between them. Harmonic distortion can also lower true power factor, so a measured PF of 0.80 does not by itself establish 675 kvar. The next slide applies this same illustrative load to a 1,000 kVA transformer.
...
- Playwright screenshots at 1280×720 and 390×844, light and dark: no runtime errors, horizontal overflow, clipped SVG labels or desktop footer overlap. Previous/next navigation connects beer → triangle → transformer comparison.
