...
- Chapter 8, slide 8 (`local-current`): regulator output now follows the selected loop resistance: 1.1 V at 100 µΩ or 1.01 V at 10 µΩ, leaving 1 V at the chip. The diagram retains voltage drop and path heat. Notes account for the additional regulator output power and distinguish the ideal 50 V / 20 A comparison.
- Chapter 8, slide 18 (`buffer-recharge`): retained unchanged at the author's request. A 40 kW time-average over 0.2 seconds represents 8 kJ; a triangular 0–80 kW profile over the same interval has the same energy but twice the peak power. The separate 0–40 kW shortfall example has 4 kJ. These are different demand profiles, not an assumed battery response delay.
...
## Integrated voltage regulation — 18 September 2026

Added the author-requested IVR distinction to Chapter 8's direct/intermediate-conversion speaker notes, the rack-conversion reading and glossary, and Chapter 9's power-stack recap notes where IVRs are named in the image. An IVR can move final regulation onto the die or into the package and replace the board VRM for the supplied rails; upstream conversion remains. Intel P226 documents compute-die FIVRs supplied by a platform regulator. No claim is made that GB300 uses this arrangement. Slide counts and diagrams remain unchanged.
