...
## 9. Transformer voltage and taps

[Hammond Power Solutions’ transformer example](https://americas.hammondpowersolutions.com/news/2014/april/how-taps-work) uses 80 primary turns and 20 secondary turns for 480 V to 120 V. The diagram follows the turns relationship: output voltage = input voltage × secondary turns / connected primary turns. At the same 480 V tap, a 504 V source gives 126 V. The 504 V tap connects 84 primary turns, producing 120 V from that source. The other illustrated primary tap ratings are 456, 468 and 492 V.

A tap selects a different number of connected winding turns. This example has no automatic voltage regulation: with its connection unchanged, the output follows input variation. The controls compare three source/connection states. Permitted input voltage depends on the equipment rating and tap; tap ratings are not an arbitrary input range. The coil symbols identify the windings; their labels give the connected turn counts.
...
The model has reached GPU memory and the GPU is computing. The electrical energy it uses becomes heat that must leave the chip. This simplified steady state accounts for 500 W of electrical input and 500 W leaving through the cold plate, with negligible other paths or stored-energy change. Power is energy transferred per second: 500 watts means 500 joules each second. Temperature tells us how hot a particular location is: the GPU is at 70 °C and the cold plate at 45 °C. Coolant flows through the plate, entering at 30 °C and leaving at 35 °C as it carries the heat away. A temperature alone does not tell us the heat-transfer rate; that also depends on the thermal path and operating conditions. The values are illustrative, not GPU or cooling-product ratings.

## 21. Cold plate, coolant and CDU
