import {scenes,initialState} from './operations-scenes.js';
import {operationsVisual} from './operations-visuals.js';
import {presentationLabels} from './teaching-navigation.js';
const $=id=>document.getElementById(id),state={...initialState,evidence:[]};
...
function render(){
 ...
 document.querySelectorAll('[data-evidence]').forEach(button=>button.onclick=()=>{const key=button.dataset.evidence;state.evidence=state.evidence.includes(key)?state.evidence.filter(item=>item!==key):[...state.evidence,key];focusAfter(`[data-evidence="${key}"]`);});
...
function fromHash(){const found=scenes.findIndex(scene=>scene.id===location.hash.slice(1));index=found<0?0:found;render();window.scrollTo({top:0,left:0,behavior:'auto'});}
