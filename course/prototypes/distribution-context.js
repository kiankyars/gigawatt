...
export function campusRoute(s,compact){
 ...
 const alt='A 138 kV grid supply feeds a campus transformer, then 13.8 kV switchgear. A hall transformer steps down again to 480 V, feeding rack and cooling branches. A separate future-hall feeder branches from the medium-voltage bus and is open.';
 if(compact){
  const x=99;let o=tx(x,28,'HV grid',23)+tx(275,28,'138 kV AC',22,power)+wire(`M${x} 46V112`,on('service'));
  o+=`<g transform="translate(${x} 160) rotate(90)">${transformer(0,0,on('campus-transformer'))}</g>`+tx(272,147,'Campus transformer',18)+tx(272,180,'138 → 13.8 kV',20);
  o+=wire(`M${x} 208V270`,on('campus-bus'))+cabinet(15,270,168,60,'MV switchgear',on('campus-bus'))+tx(272,255,'13.8 kV AC',21);
  ...
  o+=wire(`M${x} 330V423`,on('transformer'))+`<g transform="translate(${x} 470) rotate(90)">${transformer(0,0,on('transformer'))}</g>`+tx(272,506,'Hall transformer',19)+tx(272,533,'13.8 kV → 480 V',19);
 ...
 let o=tx(67,58,'HV grid',23)+tx(67,90,'138 kV AC',19,power)+wire('M25 145H241',on('service'));
 o+=transformer(288,145,on('campus-transformer'))+tx(288,58,'Campus transformer',21)+tx(288,90,'138 → 13.8 kV',19)+wire('M335 145H462',on('campus-bus'));
 o+=cabinet(462,112,174,66,'MV switchgear',on('campus-bus'))+tx(549,90,'13.8 kV AC',19,power)+wire('M636 145H739',on('transformer'));
 o+=transformer(786,145,on('transformer'))+tx(786,58,'Hall transformer',21)+tx(786,90,'13.8 kV → 480 V',19)+wire('M833 145H966',on('building-bus'));
