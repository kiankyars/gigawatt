...
const result=(value,label,m,y=400,color=C.compute)=>t(m?195:600,y,value,m?Math.min(43,340/(value.length*.56)):Math.min(66,1040/(value.length*.56)),color,'middle')+(label?t(m?195:600,y+(m?35:42),label,m?Math.min(18,345/(label.length*.52)):24,C.ink,'middle'):'');
...
function brief(s,m){
 const training=s.service==='training';let o='';
 ...
 o+=t(xx,yy,training?'Finish an experiment':'Serve requests',m?23:32,C.compute);
 const rows=training?[['Result','Reach the agreed quality target'],['Deadline','Finish the job on time'],['Include','Loading, saves and recovery']]:[['Result','Answers of the agreed quality'],['Response time','Meet the limit as requests arrive'],['Include','Queueing and bursts of traffic']];
...
function modelWork(s,m){
 const train=s.service==='training',xx=m?35:70,w=m?320:275;let o='';
 const ys=m?[85,245,405]:[170,170,170];const xs=m?[xx,xx,xx]:[70,463,855];
 const labels=train?[['Examples','Inputs + target'],['Model output','Compare with the target'],['Update parameters','Gradients + optimizer state']]:[['Request','Input'],['Run the model','Parameters + working memory'],['Response','Output']];
 ...
 if(train){if(m)o+=`<path d="M355 460H375V297H357" fill="none" stroke="${C.checkpoint}" stroke-width="3"/>`+arrow(375,297,357,297,C.checkpoint);else o+=`<path d="M992 298V379H600V298" fill="none" stroke="${C.checkpoint}" stroke-width="3"/>`+arrow(600,379,600,285,C.checkpoint)+t(790,415,'Updated model',25,C.checkpoint,'middle');}
...
function resourcePaths(s,m){
 const color=key=>s.resource===key?C.compute:C.line;let o='';
 if(m){
...
function inferenceMemory(s,m){
 const a=memoryBudget({workspaceGB:s.workspace}),x=m?28:80,w=m?334:1040;let o='';
 ...
 o+=t(x,y-24,'Example · decimal GB',m?14:18,C.muted)+t(x+w,y-24,'80 GB device',m?17:23,C.ink,'end');
 ...
 o+=result(`${a.totalGB} GB used · ${a.aggregateHeadroomGB} GB free`,'',m,m?510:437);
...
function trainingMemory(s,m){
 const a=memoryBudget({kind:'training',deviceCount:s.devices});let o='';
 ...
 o+=t(x,m?127:133,'Example memory budget · decimal GB',m?16:20,C.muted);
 o+=lines(x,m?267:280,m?['192 GB weights, gradients and optimizer','64 GB activations and workspace']:['192 GB weights, gradients and optimizer state','64 GB activations and workspace'],m?17:23,C.muted);
 ...
 o+=result(`${a.aggregateCapacityGB} GB available · 256 GB needed`,'',m,m?529:478,a.aggregateFits?C.compute:C.checkpoint);
...
function partitionFit(s,m){
 const allocation=s.partition==='balanced'?[64,64,64,64]:[96,64,48,48],a=memoryBudget({kind:'training',allocationGB:allocation});let o='';
 ...
 o+=t(m?195:600,m?85:82,'256 GB to place · 80 GB per device',m?18:25,C.muted,'middle');
 o+=t(m?195:600,m?493:458,'GB per device · dashed line = capacity',m?17:23,C.muted,'middle');
...
function measuredProgress(s,m){
 let o=t(m?195:600,m?95:116,'Example measurement · 8 devices',m?23:32,C.compute,'middle');
 ...
 o+=result('28.8 million','accepted samples at the sustained rate',m,m?319:365);
...
function observed(s,m){
 const a=utilizationCase(s.observation),x=m?40:225,w=m?310:850,scale=w/60;let o='';
 const y0=m?114:108,step=m?153:126;
 ['Allocated','System AC power','Accepted output'].forEach((label,i)=>{
 ...
 if(a.phases.length>1)o+=t(x+a.phases[0].endSeconds*scale,m?y0+step*2+104:487,`${a.phases[0].endSeconds} s`,m?16:20,C.muted,'middle');
...
function energy(s,m){
 const a=utilizationCase('A'),b=utilizationCase(s.energyCase);let o='';
 ...
 o+=t(x,y,`${label} · system AC energy over 60 s`,m?16:20,C.muted);
 const ss=v.phases.map(p=>`${p.powerKW} × ${p.seconds}`).join(' + ');
 o+=t(x,y+41,`${ss} = ${n(v.energyJ/1000)} kJ`,m?21:30)+t(x,y+68,'kW × seconds',m?14:18,C.muted);
 o+=line(x,y+79,x+w,y+79)+t(x,y+112,`${n(v.acceptedSamples)} accepted samples`,m?21:30,C.compute);
 const j=v.joulesPerAcceptedSample,txt=j===null?'Undefined':`${n(j,j===Math.round(j)?0:1)} J/sample`;
 o+=t(x,y+170,txt,m?38:51,j===null?C.muted:C.compute)+t(x,y+(m?204:220),`${n(v.averageKW,2)} kW average power`,m?17:23,C.muted);
 });
...
function batchExecution(s,m){
 const a=inferenceSchedule({batchSize:s.batch}),x=m?30:110,w=m?330:980,scale=w/40;let o='';
 o+=t(x,m?143:107,'Example timings · one instance · full queue',m?16:22,C.muted);
...
function gathering(s,m){
 const a=inferenceSchedule({arrivalIntervalMs:s.arrivals}),x=m?80:165,w=m?222:780,max=60,scale=w/max;let o='';
 ...
 const ay=m?491:458;o+=line(x,ay,x+w,ay);[0,20,40,60].forEach(v=>o+=t(x+v*scale,ay+28,String(v),m?15:19,C.muted,'middle'));o+=t(x+w+30,ay+28,'ms',m?13:17,C.muted);
 o+=t(m?30:165,m?562:78,'● Arrival / wait',m?18:23,C.checkpoint)+t(m?30:565,m?598:78,'■ Execution · 20 ms',m?18:23,C.compute);
 o+=t(m?361:1120,m?84:78,'Response',m?17:23,C.muted,'end');
...
function capacity(s,m){
 const a=inferenceSchedule({batchSize:s.batch,requestCount:8}),x=m?35:95,w=m?300:580;let o='';
 o+=t(x,m?96:107,'Arrivals · one every 6 ms',m?20:27)+r(x,m?118:131,w*166.6667/220,40,C.communication,C.communication,0)+t(x+w*166.6667/220+9,m?146:160,'166.7/s',m?16:22,C.communication);
 o+=t(x,m?214:252,'One-instance capacity',m?20:27)+r(x,m?236:276,w*a.capacityRps/220,40,C.compute,C.compute,0)+t(x+w*a.capacityRps/220+9,m?264:305,`${a.capacityRps}/s`,m?16:22,C.compute);
...
function phaseVisual(id,s,m){
 ...
 o+=t(m?20:x,m?gy-20:gy-36,m?'kW':single?'Example job power (kW)':'Sum of job power (kW)',m?16:21,C.muted)+line(x,gy+gh,x+w,gy+gh);
 ...
 if(id==='staggering-jobs')o+=lines(m?62:157,m?38:25,m?['Repeating cycle','No contention or added waiting']:['Repeating cycle · no contention or added waiting'],m?15:18,C.muted);
...
function demand(s,m){
 const mean=s.resolution==='mean',x=m?55:170,w=m?290:930,y=m?123:95,h=m?320:305,p=v=>y+h-v/550*h;let o='';
 o+=line(x,y+h,x+w,y+h)+line(x,y,x,y+h);
 if(mean){o+=line(x,p(340),x+w,p(340),C.muted,4,'9 7')+t(x+w/2,p(340)-22,'340 kW',m?32:46,C.muted,'middle');o+=t(x+w/2,y+h+36,'60-second ideal cycle',m?18:26,C.muted,'middle');}
 else{
 ...
 if(!mean)o+=t(x,y-39,'Assumed transition',m?18:23,C.muted)+t(m?195:600,m?550:507,`(160 − 480) ÷ 2 = ${transitionRate()} kW/s`,m?21:31,C.ink,'middle');
...
function acceptance(s,m){
 const a=inferenceSchedule(),v=evaluateInferenceAcceptance({schedule:a,latencyLimitMs:s.latency}),reveal=s.acceptanceReveal;let o='';
 ...
 if(reveal)o+=t(m?195:600,m?592:514,v.meetsLatency?'Both timing requirements pass':'R1 and R2 miss the limit',m?23:31,v.meetsLatency?C.compute:C.checkpoint,'middle');
...
function nextBrief(s,m){
 const mem=memoryBudget({workspaceGB:44}),p=phaseSchedule(),a=inferenceSchedule();let o='';
 const rows=[['MEMORY EXAMPLE','24 + 44 + 8 GB','80 GB device',`${mem.totalGB} GB: fits`,true],['REQUEST EXAMPLE','38 / 32 / 26 / 20 ms','Every request ≤30 ms',`${a.maxLatencyMs} ms: too slow`,false],['POWER EXAMPLE','4 jobs × 120 kW at peak','400 kW peak limit',`${p.peakKW} kW: too high`,false]];
 rows.forEach(([a,b,c,d,pass],i)=>{const x=m?27:90,yy=m?87+i*164:97+i*127,w=m?336:1020;o+=t(x,yy,a,m?14:18,C.muted)+t(x,yy+34,b,m?22:29)+t(m?x:x+560,m?yy+63:yy+34,c,m?18:25,C.muted);o+=line(x,yy+(m?85:99),x+w,yy+(m?85:99));if(s.transferReveal)o+=t(x,yy+(m?118:81),d,m?23:29,pass?C.compute:C.checkpoint);});
 return {markup:`<g data-transfer-revealed="${s.transferReveal}">${o}</g>`,description:`Three separate screens: inference memory ${mem.totalGB} GB against 80 GB; batch-four maximum response time ${a.maxLatencyMs} ms against a 30 ms every-request limit; synchronized job peak ${p.peakKW} kW against a 400 kW job-power limit. ${s.transferReveal?'Memory capacity passes, request latency fails, and peak job power fails. Carry an explicit workload and failure envelope into the supply and siting decision.':'Predict each result, then reveal the reasoning.'}`};
}
