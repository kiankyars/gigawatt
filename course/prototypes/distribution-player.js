...
function render(){const scene=scenes[index];$('title').textContent=scene.title;$('title').classList.toggle('sr-only',Boolean(scene.imageTitle));$('scenes').value=scene.id;$('progress').textContent=`${index+1} / ${scenes.length}`;$('previous').disabled=index===0;$('next').disabled=index===scenes.length-1;$('lesson-reference').href=scene.reference;controls();draw();}
