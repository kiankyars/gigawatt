import { scenes, initialState } from './distribution-scenes.js';
...
function render(){const scene=scenes[index];$('title').textContent=scene.title;$('scenes').value=scene.id;$('progress').textContent=`${index+1} / ${scenes.length}`;$('previous').disabled=index===0;$('next').disabled=index===scenes.length-1;$('lesson-reference').href=scene.reference;controls();draw();}
...
function fromHash(){const found=scenes.findIndex(scene=>scene.id===location.hash.slice(1));index=found<0?0:found;render();$('stage').scrollTop=0;window.scrollTo(0,0);}
