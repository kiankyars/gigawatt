import {scenes,initialState} from './capacity-scenes.js';
import {capacityVisual} from './capacity-visuals.js';
...
function render(){
 ...
 for(const [attribute,key,reveal]of [['upgrade-choice','upgradeChoice','showUpgrade'],['evidence-choice','evidenceChoice','showEvidence']]){
  document.querySelectorAll(`[data-${attribute}]`).forEach(button=>button.onclick=()=>{state[key]=button.getAttribute(`data-${attribute}`);state[reveal]=false;focusAfter(`[data-${attribute}="${state[key]}"]`);});
 }
