import {scenes,initialState} from './integrated-cases-scenes.js';
...
function render(){
 ...
 for(const group of scene.controls||[]){
  ...
  for(const [value,label]of group.options){
   ...
   button.onclick=()=>{state[group.key]=value;if(group.resetReveal)state[group.resetReveal]=false;render();document.querySelector(`[data-choice="${group.key}"][data-value="${value}"]`)?.focus({preventScroll:true});};
