...
const rack = rackScenes.flatMap(scene=>scene.id==='source-handoff'?[scene,loadDrop]:scene.id==='multiphase'?[scene,vrmPhaseCounts]:[scene]).map(scene=>({...scene,kind:scene.kind||'rack',...(scene.id==='board-rails'?conversionFigure:{})}));
