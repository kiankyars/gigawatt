...
const electrical = samplePresentation.steps.filter(step=>!['dc-circuit','ac-cycle','three-phase','voltage-basis'].includes(step.id)).flatMap(step=>{
  ...
  return [
    {...scene,label:'From medium-voltage AC to 800 V DC',sourceKind:'conversion-supply',explanation:step.explanation.slice(0,8)},
    {...scene,id:'ac-dc-converter-loss',label:'AC/DC converter losses',title:'How much power becomes converter heat?',sourceKind:'converter-heat',explanation:[step.explanation[4],...step.explanation.slice(8)]}
  ];
});
