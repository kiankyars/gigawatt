...
const dcBatteryRack = {
  id:'dc-battery-rack',label:'800 V DC battery rack',title:'An 800 V DC battery rack beside the compute rack',
  reference:'d04-conversion-placement',kind:'review-figure',imageTitle:true,pedagogical_role:'example',
  asset:'semianalysis-800vdc-architecture.jpeg',
  alt:'SemiAnalysis concept: medium-voltage AC is converted upstream to 800 V DC. An adjacent battery rack contains DC distribution, battery peak-shaving and capacitor peak-shaving shelves, supplying an 800 V compute rack.',
  explanation:['The AC-to-DC conversion happens upstream in this drawing. The cabinet beside the compute rack distributes DC and houses batteries and capacitors that buffer changes in demand. It is a storage and distribution cabinet, rather than the AC-fed rectifier sidecar in the earlier architecture view.','The 800 kW and Kyber/Rubin Ultra labels belong to the publisher’s proposed architecture. They are not universal rack specifications or measured operating ratings. The DC/DC distribution label alone does not establish a voltage step-down.'],
  source_ids:['SA10']
};
const dcProtection = {
...
const conversionFigure = {kind:'review-figure',imageTitle:true,asset:'rack-direct-intermediate-rome.png',
  alt:'All roads lead to Rome. Direct conversion connects a 51 V rack bus to a local 1 V converter. An intermediate-rail design converts 51 V to 12 V, then to 1 V beside the compute die. Both keep the final high-current path local.'};
...
const clementePower = {id:'clemente-power-board',label:'Meta’s GB300 power board',title:'Meta’s GB300 tray steps 51 V down to 12 V before local regulation.',reference:ledger,kind:'clemente-power',imageTitle:true,pedagogical_role:'example',
  explanation:['Meta’s Clemente compute tray is a published GB300 implementation. Its NVIDIA-designed power distribution board converts the rack bus to 12 V before supplying the compute modules. The diagram separates that tray-level conversion from local regulation at the processors.',
    'The OCP specification describes the nominal 48 V ORv3 bus in section 7.2, labels the power diagram 50 V, and specifies nominal 51 V operation with a normal 46–52 V input range in section 8.1. These are descriptions of the same low-voltage rack supply, not three serial conversion steps.',
    'The final processor voltages and regulator phase count are not specified here. The document excludes the GB300 module’s internal design. The supplied image illustrates the functional path without assigning an exact core voltage or claiming every GB300 implementation uses this intermediate rail. Its rendered board geometry is conceptual, not an OCP hardware photograph.'],
  sources:[{title:'Meta / OCP · Clemente compute tray, §§7.2 and 8.1',url:'https://www.opencompute.org/documents/clemente-compute-tray-ocp-specification-final-pdf'}]};
const vrmSwitching = {id:'vrm-switching',label:'How the phases switch',title:'Staggered switching reduces current ripple.',reference:ledger,kind:'vrm-switching',pedagogical_role:'mechanism',
  explanation:['The two switching branches belong to one voltage regulator and feed one output rail. Their switch-node voltages are rectangular pulses. Each inductor’s current rises while its upper switch conducts and falls during the rest of the cycle. The controller offsets the second phase by half a switching period.',
    'The example uses 12 V input, a 3 V output, a 25 percent duty cycle and continuous inductor current. Each phase averages 20 A with 6 A peak-to-peak ripple. With simultaneous switching, their sum ranges from 34 to 46 A: a 12 A peak-to-peak swing. Staggering reduces that to 38–42 A: a 4 A swing at the same 40 A average. Both sums share the same axes. The smaller ripple repeats each cycle rather than fading with elapsed time. Output capacitors support the remaining difference between summed inductor current and load current.',
    'This corrects the supplied image’s triangular “phase voltages”: the triangles describe inductor current, not switch-node voltage. This two-phase illustration explains the mechanism; it does not assign a phase count, rail voltage or current rating to B300. TI’s application note figures 1, 2 and 4 provide the topology and waveform distinction.'],
  sources:[{title:'Texas Instruments · Benefits of a multiphase buck converter, Figs. 1, 2 and 4',url:'https://www.ti.com/lit/an/slyt449/slyt449.pdf'}]};
const rack = rackScenes.filter(scene=>scene.id!=='rear-busbar').flatMap(scene=>{
  if(scene.id==='source-handoff')return [scene,loadDrop];
  if(scene.id==='board-rails')return [scene,clementePower];
  if(scene.id==='multiphase')return [scene,vrmSwitching,vrmPhaseCounts];
  return [scene];
}).map(scene=>({...scene,kind:scene.kind||'rack',...(scene.id==='board-rails'?conversionFigure:{})}));
...
const electrical = samplePresentation.steps.filter(step=>!['dc-circuit','ac-cycle','three-phase','voltage-basis'].includes(step.id)).flatMap(step=>{
  ...
  return [{...scene,label:'Step down first or rectify first?',title:'Should we step down first or rectify first?',kind:'review-figure',imageTitle:true,asset:'dc-step-down-or-rectify-first.png',source_ids:['P21','P22','P45'],
    ...
    explanation:['The upper path is the established transformer-plus-converter approach: first step medium-voltage AC down and provide isolation with a conventional transformer, then rectify and regulate the lower-voltage AC to 800 V DC. It combines established transformer equipment with lower-voltage power electronics. This is a practical equipment choice, not a physical requirement to step down before rectifying.','The lower path opens a solid-state transformer (SST). Its medium-voltage electronic input rectifies first; an isolated DC/DC stage uses semiconductor switching and a high-frequency transformer to step down to the DC output. A conventional transformer cannot be fed steady DC: the electronic stage creates the alternating waveform used inside the high-frequency transformer.','The internal DC links can be distributed among series-connected converter modules, which share the medium-voltage stress. The diagram groups those modules by function; it does not claim one semiconductor must withstand the entire medium-voltage input. SST topology varies.','SSTs are an available alternative, not a prerequisite for 800 V DC: Eaton lists a 2 MW medium-voltage SST with a 12.47 kV input and 800 V DC output. TI’s modular reference design explains the conversion and isolation functions; it is not this Eaton product or an 800 V product specification. Compare actual availability, qualification, protection, serviceability and whole-path efficiency for the project.','Both paths supply 800 V DC. The controlled converter establishes that voltage; a passive rectifier attached to an arbitrary AC voltage does not automatically produce a regulated 800 V output.']}];
});
...
const dcSteps = Object.fromEntries([...electrical,dcPreview,dcBatteryRack,supplement[2],greenCase,dcProtection,supplement[3],reviewFigures[1]].map(scene=>[scene.id,scene]));
...
export const dcScenes = Object.freeze([
  ...
  'dc-architecture-preview','conversion-in-rack','conversion-in-sidecar','conversion-farther-upstream','dc-battery-rack',
  'rack-bus-choices','ac-dc-ledger',
  'green-zurich-west','dc-feeder-protection','retrofit-power','ocp-power-architectures','power-stack-overview'
].map(id=>dcSteps[id]));
