/* Shared inspected circuit and waveform renderer for the sample and Chapter 8. */
export function createElectricalVisuals({ acdcWaveModel, getState, escapeHTML, fmt }) {
const phaseColors = [
  "var(--phase-one)",
  "var(--phase-two)",
  "var(--phase-three)",
...
function pairVoltageComparison() {
  const names = ["L1 − L2", "L2 − L3", "L3 − L1"],
    values = pairVoltages(getState().cycleDegrees),
  ...
  const curves = names
    .map((name, i) => {
      ...
      return `<path d="${path}" fill="none" stroke="${phaseColors[i]}" stroke-width="4" ${i === 1 ? 'stroke-dasharray="9 7"' : ""}/><circle cx="${x(getState().cycleDegrees)}" cy="${y(values[i])}" r="6" fill="${phaseColors[i]}" stroke="var(--paper)" stroke-width="2"/>`;
    })
    .join("");
  const signed = (value) =>
    Math.abs(value) < 0.05
...
function electricalContent(kind) {
  ...
  if (kind === "voltage-basis")
    return getState().voltageView === "pairs"
...
function electricalVisual(kind) {
  const scrub =
    ...
    (kind === "voltage-basis" && getState().voltageView === "pairs");
  const condition =
    kind === "dc-basics"
