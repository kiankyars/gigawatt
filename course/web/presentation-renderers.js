...
function intro() {
  ...
  const goal=getStep()?.introGoal || 'Same power delivered to the load';
  return `<div class="hero"><div class="hero-copy"><span class="chip">${escapeHTML(label)}</span><div class="hero-number">100<small> kW</small></div><span class="hero-goal">${escapeHTML(goal)}</span></div><img src="${assets}power-equipment.png" width="1672" height="941" alt="Concept illustration of conversion equipment beside a compute rack." /></div>`;
}
