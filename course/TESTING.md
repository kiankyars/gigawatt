...
## Abilene revision, Clemente VRM and Toronto additions — 2026-09-18

- Chapter 16 now has **10 scenes**. Hardware boundaries and financing clarified; cooling alternatives added; repetitive weather comparison removed with its bookmark preserved. Real Tulsa and Abilene photographs replace the factory/site drawings. The last two scenes use a campus/hall/rack recap and the actual Abilene hall. A separate ChatGPT closing-image prompt is supplied.
- Chapter 8 now has **19 scenes**. Meta Clemente's documented nominal 51 V → 12 V tray conversion precedes local regulation. The additional VRM figure distinguishes voltage pulses from inductor-current ramps; numerical tests verify duty cycle, mean currents and ripple reduction.
- Chapter 12 now has **19 scenes**. Three Toronto slides follow operating dependencies: separated lake/district-water paths, the 2013 cooling outage despite building generator operation, then the tenant's shutdown/service-transfer response.
- **297 JavaScript and 80 Python tests passed.** Course, expanded-reader, domain-map and research checks passed; `git diff --check` is clean.
- Chapter 16's seven changed/new views were captured at 1280×720 and 390×844 in light/dark modes: **28 states**, without horizontal overflow, broken images, JavaScript errors or desktop footer overlap. Independently reviewed factual claims and source/image boundaries.
- Both Chapter 8 additions passed desktop/mobile and light/dark checks (**8 states**); the three Toronto additions passed the same checks (**12 states**) plus a 1280×720 footer-fit check. Reviewed the VRM waveforms, Toronto outage, factory photos, physical-scale recap and final slide visually. Existing navigation hashes and the Chapter 12 next-chapter handoff are preserved.
- Shared speaker notes and the review tracker now distinguish implemented additions from author acceptance. Unchanged accepted chapters remain accepted.
...
## Copper-comparison scope — 2026-09-18
