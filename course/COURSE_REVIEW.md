...
| 9. [800 V DC distribution](prototypes/dc-distribution-format.html?teach=1) | 15 slides; conductor comparison, conversion placement, 50 V/800 V rack-bus paths, transformer-first versus SST supply paths and Zurich-West; closes with DC protection, retrofit, the three-column architecture comparison and the power-stack recap | [Split checks](TESTING.md#rack-power-and-dc-distribution-chapter-split--2026-09-16) | **Accepted after requested edit — 16 September 2026; reconfirmed 17 September.** Kian approved Chapter 9 upon replacing slide 10 with the supplied transformer-first/SST image. The image is installed unchanged; the 50 V versus 800 V rack-bus distinction remains. The preferred three-column comparison follows protection and the retrofit example. On 18 September, the author requested returning the power-stack map to Chapter 9 as its final recap, with the Highest ASP/server label removed from the teaching image. |
...
joins Chapter 3; local-memory versus peer traffic and model partitioning join Chapter 9
(Networking); tray repair joins Chapter 10 (Recovery). The opening illustration and
...
The earlier request’s slide numbers were matched by content: that published sequence had 23 slides; the revision has 25. Chapter 4 was subsequently accepted in Kian’s final September 14 review, recorded above.
...
The three recommended cuts are implemented following Kian's approval:

- [x] **Chapter 8, `rear-busbar`:** removed the separate repeat of Chapter 2's
  rear-rack view. The reminder remains in the rack-anatomy notes and the local
  supply path. Old links open `rack-hardware-anatomy`. The deck has 32 slides.
- [x] **Chapter 9, `tpu-interconnect`:** removed the repeated inventory. The
  following optical-routing interaction retains the Google TPU v4 identity and
...
## Chapter 16 revision and approved additions — 18 September 2026

- [x] Hardware-first design and the processor / rack-bus / AC-inlet power boundaries clarified in Chapter 16's speaker notes.
- [x] Residual-air heat defined without claiming an unverified universal Abilene RDHX layout.
- [x] Gas-generation recap retained deliberately as a delivery and continuity connection.
- [x] Concise outdoor-cooling choice added before the Abilene design; cost scope clarified.
- [x] Repetitive hot-weather slide removed, with its old link preserved.
- [x] Factory/site sketches replaced with real photographs and Crusoe's documented 2,500+ switchboards.
- [x] Campus funding and the hyperscaler lease clarified.
- [x] Two-slide closing sequence and [ChatGPT image prompt](CHAPTER_16_CLOSING_IMAGE_PROMPT.md) completed.
- [x] Approved Chapter 8 Clemente and VRM additions implemented; the mislabeled supplied waveform concept is redrawn with correct voltage/current labels.
...
- [x] Power triangle audit followed by explicit author authorization: one concise slide now follows the Chapter 6 numerical comparison (slide 25).

### Supplied figures, closing order and investor explanation — 18 September 2026

- [x] Chapter 12: Toronto moved to slides 16–18; resource dependencies closes at slide 19 with the shared next-chapter button.
- [x] Chapter 16 slide 5 title: “A potential example of greenwashing?”; notes distinguish a discussion question from an established finding.
- [x] Chapter 16 slide 7 title: “A Hierarchy of Deals”; investment returns tied to the physical facility’s lease income and value, with undisclosed partner economics left unspecified.
- [x] Chapter 8: supplied Clemente image replaces the diagram and duplicate title; existing OCP citation retained.
- [x] Chapter 16: supplied closing image appended; both existing photographic closing slides preserved.
- [x] Chapter 6: requested power-triangle slide placed after the comparison that introduces its numbers.


### Finale speaking purpose and hall-photo placement — 18 September 2026

- [x] Chapter 16's three consecutive closing slides reduced to two: slide 9 connects rack, hall and campus; slide 10 delivers the closing line. Short spoken scripts are in the player notes and `SPEAKER_NOTES.md`.
- [x] Full-slide Abilene hall photograph moved to Chapter 2, immediately before the coolant-loop diagram. The earlier white/gray-space photograph retains its room-layout purpose.
- [x] The old Chapter 16 `#abilene-data-hall` bookmark resolves to the physical-scale recap. Current chapter counts are 14 and 10; these edits await author review.


### Reuse the Abilene photograph in the cooling lesson — 18 September 2026
