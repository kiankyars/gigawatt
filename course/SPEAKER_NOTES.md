...
The input specification gives 51 V nominal and a 46–52 V normal operating range. The document also uses 48/50 V names for the rack supply; those are not additional conversion stages. It does not establish an exact B300 core voltage or phase count. One multiphase VRM can contain many switching branches feeding the same rail.
...
7. **Name the roles.** Crusoe builds and operates the physical campus. **Oracle is the hyperscaler/customer here** and supplies the GPU cloud infrastructure. OpenAI uses that capacity for training and inference. Crusoe’s June 2026 announcement explicitly calls Abilene purpose built for Oracle. The original 2024 lease announcement left the tenant unnamed; it did not disclose the exact legal lessee or contract terms. Crusoe, Blue Owl and Primary Digital fund the campus venture. Keep that funding separate from Oracle’s GPU purchases and OpenAI’s compute bill. The slide replaces the ambiguous lease caption with the three named roles.
...
10. **From Watts to Tokens.** Close inside the actual Abilene data hall. Suggested final line: “A token depends on much more than a chip. You can now follow its physical requirements back through the rack, the cooling plant, the electrical supply, and the decisions that made the campus possible.” The campus's MW rating alone cannot establish a token rate. A [ready-to-paste ChatGPT image prompt](CHAPTER_16_CLOSING_IMAGE_PROMPT.md) provides an optional conceptual final illustration with room for the course title.
